...
RSpec.describe 'Inventory', type: :system do
  it 'goes to inventories page' do
    visit '/users/sign_in'
    User.create!(name: 'example', email: 'example@example.com', password: 'password', confirmed_at: Time.now)
  ...
  it 'adds food to an inventory' do
    ...
    food1 = user.foods.create!(name: 'apple', measurement_unit: 'grams', price: 3)
    user.foods.create!(name: 'meat', measurement_unit: 'kg', price: 12)
    ...
    find(:css, '#inventory_food_food_id').find(:option, 'apple').select_option
  ...
  it 'removes food from inventory' do
    ...
    inventory = user.inventories.create!(name: 'Inventory 1')
    food = user.foods.create!(name: 'apple', measurement_unit: 'kg', price: 3)
